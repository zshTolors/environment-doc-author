...
基于当前机器的真实环境状态，为依赖本机命令的 agent 任务提供事实校验，并生成、维护可复用的环境基线文档。
这个仓库提供两套独立实现：一套纯 Node.js，一套 Python。两者都可以探测当前环境、维护机器可读的 baseline、帮助 agent 基于已验证事实选择本机相关命令，并渲染 `ENVIRONMENT_POLICY.md`、`AGENTS` 片段和可复用的 skill 片段。
...
| macOS | JavaScript | 是 | 已在 macOS 26.2（Apple silicon）和 Node.js 24.14.0 上完成端到端实测 |
| macOS | Python | 是 | 已在 macOS 26.2（Apple silicon）和 Python 3.9.6 上完成端到端实测 |
| Linux | JavaScript | 是 | 已在 Ubuntu 22.04 上用 Node.js 20 完成端到端实测 |
| Linux | Python | 是 | 已在 Ubuntu 22.04 上用 Python 3.10 完成端到端实测 |

## 说明

- 如果仓库里已经存在 `AGENTS.md`，默认不要直接覆盖。应优先生成或刷新独立的环境片段文件，例如 `AGENTS.environment.md`；只有在用户明确要求时，才合并进现有 `AGENTS.md`。
